from google import genai
import time

MODELS_TO_TRY = [
    "gemini-2.0-flash-lite",
    "gemini-2.0-flash",
    "gemini-1.0-pro",
]

def generate_kids_script(gemini_api_key, topic, language="English", retries=2):
    client = genai.Client(api_key=gemini_api_key)
    prompt = f"""Create a 20-30 second kids educational nursery rhyme for YouTube Shorts.
Topic: {topic}
Language: {language}
Safe for ages 1-6, simple, repetitive, catchy, and educational.

Return in this exact format:
TITLE: [catchy title]
LYRICS: [rhyme lyrics]
SCENE: [simple animation description]
TAGS: [5 comma separated tags]"""

    for model in MODELS_TO_TRY:
        for attempt in range(retries):
            try:
                response = client.models.generate_content(
                    model=model,
                    contents=prompt,
                )
                print(f"Script generated using model: {model}")
                text = response.text
                result = {}
                for line in text.split("\n"):
                    for key in ["TITLE", "LYRICS", "SCENE", "TAGS"]:
                        if line.startswith(f"{key}:"):
                            result[key.lower()] = line.replace(f"{key}:", "").strip()
                return result
            except Exception as e:
                err = str(e)
                if "404" in err:
                    print(f"Model {model} not available, trying next...")
                    break
                print(f"Attempt {attempt + 1} failed: {e}")
                if attempt < retries - 1:
                    time.sleep(20)

    print("All models failed. Using fallback script.")
    return {
        "title": f"Fun {topic[:30]} Song",
        "lyrics": f"Let us learn about {topic[:30]} today, sing and dance and laugh and play!",
        "scene": "Colorful animated characters dancing",
        "tags": "kids,educational,shorts,divijoytoons,rhymes",
    }
